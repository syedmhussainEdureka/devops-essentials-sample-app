...
"""
Runs Coverity Prevent on a build of .
...
# Relative to _SOURCE_DIR.
SOLUTION_FILE = 'src\\chrome\\chrome.sln'

# Relative to SOURCE_DIR.
SOLUTION_DIR = '.'

COVERITY_BIN_DIR = 'C:\\Program Files\\Coverity\\Coverity Static Analysis\\bin'

COVERITY_INTERMEDIATE_DIR = 'C:\Temp\J0015_Sensor_FPGA\cov-build'
...
# Relative to SOURCE_DIR.  Contains the pid of this script.
...
def main(options, args):
  ...
  # running.
  lock_filename = '%s\\%s' % (SOURCE_DIR, LOCK_FILE)
  try:
  ...
  print ('Change directory to ' + SOURCE_DIR)
  os.chdir(SOURCE_DIR)
  ...
  # trunk/tools/buildbot/scripts/common/utils.py.
  cmd = 'rmdir /s /q %s\\%s\\%s' % (SOURCE_DIR,
                                    SOLUTION_DIR, options.target)
  _RunCommand(cmd, options.dry_run, shell=True)
  print ('Elapsed time: %ds' % (time.time() - start_time))
	
  cmd = '%s\\cov-build.exe --dir %s devenv.com %s\\%s /build %s' % (
	      COVERITY_BIN_DIR, COVERITY_INTERMEDIATE_DIR, SOURCE_DIR,
	      SOLUTION_FILE, options.target)
