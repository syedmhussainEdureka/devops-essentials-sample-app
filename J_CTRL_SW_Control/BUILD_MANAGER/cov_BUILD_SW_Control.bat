...
set srcDIR=%csciBASE%\%CSCI%\%rlsNUM%\ARM_ONLY\omap_arm\src
...
set wkspDIR=%csciBASE%\%CSCI%\%rlsNUM%\ARM_ONLY
echo WKS: %wkspDIR%
echo CLEAN ALL
rmdir /S /Q %srcDIR%
rmdir /S /Q %bldDIR%
cd %wkspDIR%
echo %covEXE%\cov-build.exe" --no-log-server --config %covCFG% --dir %bldDIR%  c:\ti\ccsv5\eclipse\eclipsec -noSplash -data %wkspDIR% -application com.ti.ccstudio.apps.projectBuild -ccs.projects %projDMY% -ccs.clean -ccs.configuration %bldTYP% -ccs.autoImport
%covEXE%\cov-build.exe" --no-log-server --config %covCFG% --dir %bldDIR%  C:\ti\ccsv5\eclipse\eclipsec -noSplash -data %wkspDIR% -application com.ti.ccstudio.apps.projectBuild -ccs.projects %projDMY% -ccs.clean -ccs.configuration %bldTYP% -ccs.autoImport 
...
echo WKS: %wkspDIR%
cd %wkspDIR%
echo %covEXE%\cov-build.exe" --no-log-server --config %covCFG% --dir %bldDIR% C:\ti\ccsv5\eclipse\eclipsec -noSplash -data %wkspDIR% -application com.ti.ccstudio.apps.projectBuild -ccs.projects %projDMY% -ccs.buildType full -ccs.configuration %bldTYP% -ccs.autoImport
%covEXE%\cov-build.exe" --no-log-server --config %covCFG% --dir %bldDIR% C:\ti\ccsv5\eclipse\eclipsec -noSplash -data %wkspDIR% -application com.ti.ccstudio.apps.projectBuild -ccs.projects %projDMY% -ccs.buildType full -ccs.configuration %bldTYP% -ccs.autoImport 
:: %covEXE%\cov-build.exe" --add-arg --c99 --delete-stale-tus --return-emit-failures --fs-capture-search  --dir %bldDIR% C:\ti\ccsv5\eclipse\eclipsec -noSplash -data %wkspDIR% -application com.ti.ccstudio.apps.projectBuild -ccs.projects %projDMY% -ccs.buildType full -ccs.configuration %bldTYP% -ccs.autoImport 
...
set bldDIR="C:\Temp\%CSCI%\cov-build"
echo STEP3
echo %covEXE%\cov-analyze.exe" --dir %bldDIR%  --config %covCFG% --all --enable-fnptr --strip-path %csciBASE%\%CSCI%\%rlsNUM%\ARM_ONLY --aggressiveness-level medium --enable-fb %DisableCheckers% --enable-parse-warnings !ParseWarnCmd!
...
:: %covEXE%\cov-analyze.exe" --dir %bldDIR% --strip-path %csciBASE%\%CSCI%\%rlsNUM%\ARM_ONLY --aggressiveness-level high --enable-fb %DisableCheckers% --enable-parse-warnings !ParseWarnCmd!
