...
if OS=="nt":
   rlsNUM="INT"
   CSCI="J_CTRL_SW_Control"
   ProjectLIST=['omap_arm','bareMetalDSP']
   projDMY="omap_arm"
...
##+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    if os.path.exists(bldDIR):
       exit_status=shutil.rmtree(bldDIR)
       ##my_print("EXT: ", exit_status)
       if exit_status == 1:
          ExtErr(bldDIR, exit_status)       
    
    wkspDIR=csciBASE + "\\" + CSCI + "\\" + rlsNUM + "\\" + "WORKSPACES" + "\\"
    my_print ('WKS: ', wkspDIR)
    exit_status = os.chdir(wkspDIR)
    osCMD=(bldEXE + " --no-log-server --config " + covCFG + " --dir " + bldDIR +  " C:\\ti\\ccsv5\\eclipse\\eclipsec -noSplash -data " + wkspDIR + " -application com.ti.ccstudio.apps.projectBuild -ccs.projects " + par + " -ccs.clean -ccs.configuration  Release -ccs.autoImport" )
...
##+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    print ('WKS: ', wkspDIR)
    exit_status = os.chdir(wkspDIR)
    osCMD=(bldEXE + " --no-log-server --config " + covCFG + "--dir " + bldDIR +  " C:\\ti\\ccsv5\\eclipse\\eclipsec -noSplash -data " + wkspDIR + " -application com.ti.ccstudio.apps.projectBuild -ccs.projects " + par + " -ccs.buildType full -ccs.configuration  Release -ccs.autoImport" )
    exit_status = subprocess.check_call(osCMD)
    if exit_status == 1:
       ExtErr(osCMD, exit_status)
...
##+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    osCMD=(anzEXE + " --dir " + bldDIR + " --config " + covCFG + " --all --enable-fnptr --strip-path " + csciBASE + "\\" + CSCI + "\\" + rlsNUM + "\\" + '\\ARM_ONLY ' + ' --aggressiveness-level medium ')
...
for i in (ProjectLIST): 
    ...
    anzyleProj(i)
    raise SystemExit()
    anzyleProj('bareMetalDSP')
    cleanProj('bareMetalDSP')
    buildProj('bareMetalDSP')
    raise SystemExit()
